...
def tx(x,y,w,h,text,size,color,bold=False,align="left",font="Noto Sans CJK JP"):
...
for theme, theme_label in THEMES:
    ...
    for cid,title,content in COMPONENTS:
        ...
        e.append(tx(100,173,1000,22,cid,10,muted,True,font=font))
        ...
        e.extend(r.elements)
        e.append(tx(100,1018,1300,20,f"{theme_label} — composition, typography and reading path are theme-specific",10,muted,False,font=font))
        e.append(tx(1740,1016,80,22,f"{n} / {len(THEMES)*len(COMPONENTS)}",10,muted,True,"right",font))
        slides.append({"layout":"Title Only","placeholders":{"0":""},"background":bg,"elements":e})

deck={"template":"blank-light.pptx","fonts":{"fullwidth":"Noto Sans CJK JP","halfwidth":"Aptos"},"defaultTextColor":"#123039","slides":slides}
out=ROOT/'examples'/'premium-components-gallery-v4.json'; out.write_text(json.dumps(deck,ensure_ascii=False,indent=2),encoding='utf-8'); print(out)
